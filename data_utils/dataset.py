...
def load_data_test(args):
    ...
    sk_valid_dat = ValidSet(pre_load, 'sk', half=True)

    im_valid_data = ValidSet(pre_load, 'im', half=True)

    return sk_valid_dat, im_valid_data
...
def load_data(args):
    ...
    sk_valid_dat = ValidSet(pre_load, 'sk')

    im_valid_data = ValidSet(pre_load, 'im')

    return train_data, sk_valid_dat, im_valid_data
...
class TrainSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        n_label = self.class_dict.get(self.choose_label_name[-1])
        # # sketch
        ...
        return sketch, image, sketch_neg, image_neg, sk_label, im_label, sk_label_neg, n_label
        # return sketch, image, sketch_neg, image_neg, sk_label, im_label, sk_label_neg, n_label, sk_name, im_name, imn_name
...
class ValidSet(data.Dataset):
    ...
    def __init__(self, pre_load, type_skim='im', half=False):
        ...
        # print("file:", len(self.file_names))
        # print("cls:", len(self.cls))
