...
class Relation_Network(nn.Module):
    def __init__(self, anchor, dropout=0.1):
        ...
        # 0.9
            ...
            self.rn = nn.Sequential(
                nn.Linear(anchor * anchor, 216, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
        ...
        # 0.8
            ...
            self.rn = nn.Sequential(
                nn.Linear(anchor * anchor, 1024, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
                nn.Linear(1024, 101, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
                nn.Linear(101, 1, bias=True)
            )
        ...
        # 0.5
            ...
            self.rn = nn.Sequential(
                nn.Linear(anchor * anchor, 125, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
                nn.Linear(125, 25, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
