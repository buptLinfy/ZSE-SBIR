...
class Encoder(nn.Module):
    def __init__(self, input_shape, num_layers, heads, mlp_dim, inputs_positions=None, dropout_rate=0.1, train=False):
        ...
        self.keep_rate = (1, ) * 12
        # self.keep_rate = (0.9, ) * 12   # 196 -> 59
        # self.keep_rate = (1, 1, 1, 0.9) + (1, 1, 0.9) + (1, 1, 0.9) + (1, 1)    # 196 -> 177 -> 160 -> 144
        # self.keep_rate = (1, 1, 1, 0.8) + (1, 1, 0.8) + (1, 1, 0.8) + (1, 1)    # 196 -> 157 -> 126 -> 101
        # self.keep_rate = (1, 1, 1, 0.7) + (1, 1, 0.7) + (1, 1, 0.7) + (1, 1)    # 196 -> 138 -> 97 -> 68
        # self.keep_rate = (1, 1, 1, 0.5) + (1, 1, 0.5) + (1, 1, 0.5) + (1, 1)    # 196 -> 98 -> 49 -> 25
...
class ViTPatch(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, depth, heads, mlp_dim, channels=3, dropout=0.,
                 emb_dropout=0.):
        super().__init__()
...
class Self_Attention(nn.Module):
    def __init__(self, d_model, cls_number=100, pretrained=True):
        ...
        if pretrained:
            checkpoint = torch.load("/root/lfy/HANet/model/sam_ViT-B_16.pth")
